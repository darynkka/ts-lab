...
class App {
  ...
  initEventListeners = () => {
    ...
    document.getElementById('booksList')?.addEventListener('click', this.handleBookAction);
  };
  ...
  createBookListItem = (book: IBook) => {
    ...
    return `
          <li class="d-flex justify-content-between align-items-center mb-2">
              ...
              <button class="btn ${buttonClass} btn-sm" data-book-id="${book.id}" data-action="${
  ...
  showBorrowBookModal = (bookId: string) => {
    Modal.showBorrowBookModal(bookId, (userId: string) => this.borrowBook(bookId, userId));
  };
  ...
  borrowBook = (bookId: string, userId: string) => {
    try {
      ...
      if (
        ...
        Modal.showErrorModal('Помилка позичення книги', 'Не вдалося позичити книгу.');
      }
  ...
  returnBook = (bookId: string) => {
    try {
      ...
      Modal.showErrorModal('Помилка повернення книги', 'Не вдалося повернути книгу.');
    }
  };
